...
export const handleCreateView = async (viewname = "", viewjson: any) => {
    const query = `CREATE TABLE ${viewname.toLowerCase()} (prim_id text PRIMARY KEY);`;
    await makeRequest(query);
    const updateViewsTable = `INSERT INTO schemas(id, viewname, schema_object) VALUES ('a${Math.round(Math.random() * 10000000)}', '${viewname.toLowerCase()}', '${JSON.stringify(viewjson)}');`;
    await makeRequest(updateViewsTable);
}

export const handleSubViewCreate = async (viewname: string, subviewname: string, subviewtype: string, updatedSchema: any) => {
